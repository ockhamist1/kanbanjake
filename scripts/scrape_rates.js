...
async function scrapeLender(lender, today) {
    ...
    try {
        ...
        // Individual 60-second timeout per lender
                ...
                if (decimals) {
                    ...
                    if (rawRate) {
                        ...
                        localResults.push({
                            ...
                            rate: format3(rawRate), apr: format3(rawApr), points: format3(rawPoints),
                            timestamp: admin.firestore.FieldValue.serverTimestamp()
                        });
                        console.log(`   ✅ ${lender.name} ${conf.id}: ${rawRate.toFixed(3)}%`);
                    }
                }
            }
        }
...
async function run() {
    ...
    const allResults = await Promise.all(LENDERS.map(l => scrapeLender(l, today)));
    const flatResults = allResults.flat();

    if (flatResults.length > 0) {
        console.log(`>>> SAVING: ${flatResults.length} records to Firebase.`);
        ...
        console.error(">>> NO DATA FOUND. CHECK SCRAPERAPI CREDITS.");
    }
}
